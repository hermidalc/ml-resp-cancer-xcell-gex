...
parser$add_argument(
    "--feat-file", type="character", nargs=1, help="feature file"
)
parser$add_argument(
...
if (args$filter == "common_features") {
    ...
    feat_file_basename_parts <- strsplit(
        basename(args$feat_file), split="_", fixed=TRUE
    )[[1]]
    filt_type <- paste(
        feat_file_basename_parts[1:length(feat_file_basename_parts) - 1],
        collapse="_"
    )
                    ...
                    for (prep_meth in prep_methods) {
                        for (bc_meth in bc_methods) {
                            ...
                            if (!exists(eset_tr_name)) {
                                ...
                                common_feature_names <- intersect(
                                    featureNames(eset_tr_filt),
                                    feature_names
                                )
                                cat(
                                    "  Filter:",
                                    length(common_feature_names), "/",
                                    length(feature_names),
                                    "features\n"
                                )
                                eset_tr_filt <-
                                    eset_tr_filt[common_feature_names, ]
                            ...
                            for (dataset_te_name in setdiff(
                                dataset_te_names, dataset_tr_name_combos[, col]
                            )) {
                                if (
                                    length(dataset_tr_name_combos[, col]) > 1 ||
                                    bc_meth != "none"
                                    ...
                                    eset_te_name <- paste0(
                                        c(eset_tr_name, dataset_te_name, "te"),
                                        collapse="_"
                                    )
                                    eset_te_filt_name <- paste0(c(
                                        eset_tr_filt_name, dataset_te_name, "te"
                                    ), collapse="_")
                                } else {
                                    # eset_te_name <- paste0(c(
                                    #     "eset", dataset_te_name, suffixes_te
                                    # ), collapse="_")
                                    # eset_te_filt_name <- paste0(c(
                                ...
                                if (!exists(eset_te_name)) {
                                    ...
                                    eset_te_filt <-
                                        eset_te_filt[common_feature_names, ]
                                    assign(eset_te_filt_name, eset_te_filt)
                                    save(list=eset_te_filt_name, file=paste0(
                                        "data/", eset_te_filt_name, ".Rda"
                                    ))
